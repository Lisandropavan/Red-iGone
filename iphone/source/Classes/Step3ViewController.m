...
#define API_APP_NAME @"Red iGone for iPhone"
#define API_APP_VERSION @"1.0.1"
...
#define API_UPLOAD_FILENAME @"rig_iPhone.jpg"
...
#define SPINNER_POSTION_X 160
#define SPINNER_POSITION_Y 190
...
/*
 *	Downloads the selected image from the API server
 */
- (void) downloadImage {
	GlobalData * globalData = [GlobalData sharedInstance];

		self.downloadAlert = [[[UIAlertView alloc] initWithTitle:SAVING_IN_PROGRESS_TITLE message:nil delegate:self cancelButtonTitle:nil otherButtonTitles: nil] autorelease];
		[self.downloadAlert show];
...
- (void) showConnectionError {
	...
	self.downloadAlert = [[[UIAlertView alloc] initWithTitle:CONNECTION_ERROR_TITLE
  ...
  if (DEBUG) {
    ...
    if (DEBUG) {
      ...
      NSLog(@"Hide spinner and show thumb image");
		...
		//Reposition the view
		CGPoint newPoint = CGPointMake(globalData.imagePositionX, globalData.imagePositionY+20);
...
- (void)viewDidLoad {
  if (DEBUG) {
	...
	self.internetReachable = [[Reachability reachabilityForInternetConnection] autorelease];
