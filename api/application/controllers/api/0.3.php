...
class ApiController extends Controller {
	...
	public function generate_api_key() {		
		...
		try {
			...
			$res = $this->model->insert_key($data);
	...
	public function validate_api_key() {
		...
		try {
			$res = $this->model->validate_key($key);
			
			if ($res['active']) {
				
				$sid = md5(uniqid());
				$res = $this->model->insert_session_id($sid);
				if (!$res) {
					$error_msg['error'] = 'Failed to create session id';
				}
	...
	protected function validate_session_id($session_id) {
		$this->label = 'RiG';
		$this->version = '0.3';
		...
		if (!empty($session_id)) {			
			try {
				$res = $this->model->validate_session_id($session_id);
